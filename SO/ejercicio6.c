...
//codigo para imprimir fichero con exec()
    if (hijo_pid == -1) {
        perror("fork error para cat");
        exit(EXIT_FAILURE);
    }
    else if (hijo_pid == 0) { // hijo que ejecuta cat
        ...
        // espera a que termine el cat
        waitpid(hijo_pid, &status, 0);
    }
